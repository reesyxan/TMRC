...
fit_1var <- glm(target_train_binary ~ train[,top25DEgenes_down[1]], family=binomial, data= data.frame(train[,top25DEgenes_down[1]]))
fit_2vars <- glm(target_train_binary ~ train[,top25DEgenes_down[2]], family=binomial, data= data.frame(train[,top25DEgenes_down[2]]))
fit_3vars <- glm(target_train_binary ~ train[,top25DEgenes_down[3]], family=binomial, data= data.frame(train[,top25DEgenes_down[3]]))
...
qplot(train_DEfeatures[,1], round(predictions_1var), col=target_train, xlab = colnames(train_DEfeatures)[1], ylab = 'Prediction using Logistic Reg.')
plot(fit_1var)

qplot(train_DEfeatures[,1], round(predictions_2var), col=target_train, xlab = colnames(train_DEfeatures)[2], ylab = 'Prediction using Logistic Reg.')

qplot(train_DEfeatures[,1], round(predictions_3var), col=target_train, xlab = colnames(train_DEfeatures)[3], ylab = 'Prediction using Logistic Reg.')

writeLines("\nNot bad!  Of our 28 samples, only  2were misclassified as cure when they were actually failure. My guess is we would rather false negatives (were actually cure when we predict they would be failure to cure). Is this correct?")
```
...
topLDA_features <- head(train_LDA$scaling[order(abs(train_LDA$scaling), decreasing = TRUE),])
...
fit_LDAvars <- glm(target_train_binary ~ ENSG00000281039 + ENSG00000259371 + ENSG00000268797 + ENSG00000282804 + ENSG00000272410 + ENSG00000283977, family=binomial, data= LDAgenessubset)
...
writeLines(paste("\nThese two genes are numbers", which(top25DEgenes_down == "ENSG00000282804"), "and", which(top25DEgenes_down == "ENSG00000272410"), "in the DE gene order."))


writeLines("\nSo this model fit is a bit better than what we got using the just the top DE genes and is better than the mean AIC of the random subset of top DE genes. So far, this method of feature selection is the most optimal.")
