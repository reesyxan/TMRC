...
```{r, include=FALSEz}
writeLines("\nI am not sure why the top Negative FC genes have such outliers. This doesn't seem right to me so something to talk about on Wednesday. For now, I'll just use the top pos DE genes.")
```


# Logistic Regression Model
```{r}
train_DEfeatures <- as.data.frame(train[,top25DEgenes_up[1:25]])

fit_1var <- glm(target_train_binary ~ train[,top25DEgenes_up[1]], family=binomial, data= data.frame(train[,top25DEgenes_up[1]]))
fit_2vars <- glm(target_train_binary ~ train[,top25DEgenes_up[2]], family=binomial, data= data.frame(train[,top25DEgenes_up[2]]))
fit_3vars <- glm(target_train_binary ~ train[,top25DEgenes_up[3]], family=binomial, data= data.frame(train[,top25DEgenes_up[3]]))
