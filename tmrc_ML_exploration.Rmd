...
hist(probabilities_1var, breaks = 15)
hist(probabilities_2var, breaks = 15)
hist(probabilities_3var, breaks = 15)
```
...
writeLines(paste("Hmmm, so this isn't great. This is showing us that if we take random subsets of genes from the top 25 genes, the mean AIC for those models is", round(mean(AICs),2), ", so our models using only the top 3 DE genes really isn't doing any better than a model taking a random subset of 3 genes in the top 25 DE genes. So either we have some really good top 25 DE genes on a logit model isn't the best for us. Let's try an SVM."))
